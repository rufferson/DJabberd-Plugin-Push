...
my %respmap = (
    'error-{jabber:server}error' => \&error,
    'result-(BOGUS)' => \&result
);
sub register {
    my ($self,$vhost) = @_;
    ...
    my $reply_cb = sub {
        my ($vh, $cb, $iq) = @_;
        if(exists $respmap{$iq->signature}) {
            return $cb->stop_chain
                if($respmap{$iq->signature}->($self,$iq));
        }
        $cb->decline;
    };
    $self->{vhost} = $vhost;
    ...
    # Register S2S IQ Handler to catch responses
    $vhost->register_hook("s2s-iq",$reply_cb);
...
sub enable {
    ...
    if($el->first_element) {
        ...
        $logger->debug("Form element: ".$form->as_xml);
        $logger->debug("Form type: ".$form->form_type);
        $form = undef if($form->form_type ne 'http://jabber.org/protocol/pubsub#publish-options');
    }
    $logger->debug("Enabling Push Notifications for ".$jid->as_string." to $srv as $node with ".$form);
...
sub wrap {
    ...
    my $ret = DJabberd::IQ->new('','iq',
        { '{}from' => $from, '{}to' => $to, '{}type' => 'set'},
        [ DJabberd::XMLElement->new(PUBSUB,'pubsub',{xmlns=>PUBSUB}, [
            DJabberd::XMLElement->new(PUBSUB,'publish',{node=>$node},[
                DJabberd::XMLElement->new(PUBSUB,'item',{},[
                    DJabberd::XMLElement->new(PSHNv0,'notification',{xmlns=>PSHNv0},[],$form->as_xml())
                ])
            ])
          ])
        ]
...
sub handle {
    ...
    foreach my$push(@to) {
        ...
        my $pub = $self->wrap($form,$self->vh->server_name,$push->[0],$push->[1]);
        if($push->[2] && ref($push->[2])) {
            $pub->first_element->push_child(DJabberd::XMLElement->new(undef,'publish-options',{},[],$push->[2]->as_xml));
        } else {
            $logger->debug("No push options in ".$push->[2]." which is unusual");
        }
        # Add to pending response queue, should be cleaned by error/result response or timeout
        $self->{idq}->{$pub->id} = [ $to->as_bare_string, $to->as_string, time, $push->[0] ];
        $pub->deliver($self->vh);
        # Schedule garbage collection in free slot
        Danga::Socket->AddTimer(0,sub {
            for my$id (keys(%{$self->{idq}})) {
                if((time - $self->{idq}->{$id}->[2]) > 900) {
                    # No ack/nack in 15min? Garbage
                    delete $self->{idq}->{$id};
                }
            }
        });
...
sub result {
    my ($self, $iq) = @_;
    if($iq->to eq $self->vh->server_name && exists $self->{idq}->{$iq->{id}} && $self->{idq}->{$iq->{id}}->[3] eq $iq->from) {
        $logger->debug("Got response: ".$iq->as_xml);
        # Just discard pending reply
        return delete $self->{idq}->{$iq->{id}};
    }
}

sub error {
    my ($self, $iq) = @_;
    if(exists $self->{idq}->{$iq->id} && ref($self->{idq}->{$iq->id}) && ref($self->{reg}->{$self->{idq}->{$iq->id}->[0]})) {
        $logger->debug("Got error: ".$iq->as_xml);
        # Discard pending reply and push enablement
        my $data = delete $self->{idq}->{$iq->id};
        delete $self->{reg}->{$data->[0]}->{$data->[1]};
        return 1;
    }
    return undef;
}
